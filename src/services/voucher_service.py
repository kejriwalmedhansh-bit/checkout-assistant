...
from ..repositories import platform_rules_repository as platform_rules
...
# Platform behaviour is NOT defined here. It lives in data/voucher_rules.json
# under `_platform_rules`, generated by scripts/build_service_rules.py, and is
# read through platform_rules_repository. Two copies of one fact is how a
# ₹29,999 Frido order came to be recommended as eight separate purchases: the
# rulebook said Maximize sells one item per checkout, a hand-typed constant in
# this file said otherwise, and the constant won silently.
#
# Kept only as the set of platform names for code that still asks the question
# as a set membership; prefer platform_rules.sells_one_item_per_checkout().
_SINGLE_ITEM_CHECKOUT_PLATFORMS = {
    name for name in ("gyftr", "maximize", "buyhatke")
...
# One. Confirmed by the product owner 2026-09-07 as the single governing rule:
# "Dealo only ever recommends a plan that can be bought in ONE checkout, and
# anything not covered is paid normally by card." Nobody is sent to buy twice,
# whatever the rate — the card is the release valve, not a second errand.
MAX_CHECKOUTS = 1
    ...
    tiers = [
        {
            **record, **p,
            ...
            # Gated on the BRAND's own terms, not on a per-listing number
...
def _store_allows_stacking(merchant_name: str) -> bool:
    ...
    Updated 2026-09-07: the standardised rules extracted from the sellers' own
    terms answer this directly, for all three platforms, with the sentence they
    came from — `can_combine`, present on 1,536 listings and definite on 77% of
    them. That is a better source than inferring it from one platform's scrape,
    so it is consulted first; the original Gyftr signal remains as a fallback
    for the listings whose terms never said.
    """
    for source in ("gyftr", "maximize", "buyhatke"):
        slug = _slug_for(source, merchant_name)
        if not slug:
            continue
        rec = _get_voucher_rules(source, slug)
        answer = ((rec or {}).get("rules") or {}).get("can_combine", {}).get("value")
        if answer in ("yes", "no"):
            return answer == "yes"
...
def _pick_best_candidate(candidates: list[tuple]) -> tuple:
    ...
    short_enough = cheapest[0].get("txns_needed", 1) <= MAX_CHECKOUTS
