...
6. **Text-query mode has no identity verification** — matcher runs only in URL mode; outlier filter is the sole guard. Flagged for a dedicated L1 fine-tuning session (variant/colour/size extraction accuracy).
7. `_KNOWN_BRANDS` (pipeline.py) and `_BRAND_SLUGS` (discovery.py) are small hardcoded lists — brands outside them weaken brand inference and conflict filtering.
8. Only `amzn.in` short links resolve; `fkrt.co` / `bit.ly` will fail extraction.
9. WhatsApp links are not Cuelinks-wrapped (web links are). Decide whether that's intentional before production.
10. No automated tests. Regression set is manual: boAt Airdopes 141, Myntra Nike sneakers URL, Ray-Ban Meta Wayfarer Gen 2, Lakme CC Cream, amzn.in short link.
