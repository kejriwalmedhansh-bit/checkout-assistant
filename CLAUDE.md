...
6. **Text-query mode Product Picker no longer checks title relevance or filters accessories — deliberate, not a regression (2026-08-03).** `_filter_and_group_candidates` in `search_service.py` used to require every search word to appear in a listing's title (four "rungs" of matching, from exact to relaxed) and to drop accessory-titled listings (case/cover/charger/etc.) before anything reached the picker. Both were removed by explicit user decision after a debugging session found the matching rungs wrongly dropped a genuine listing (a title like "Buds4 Pro" with no space before the digit failed the "4" word-boundary check) and the accessory filter wrongly suppressed accessory-shaped searches (e.g. "iPhone 15 case" had every case listing dropped). Accepted trade-off: the picker now only gates on trusted-seller + price-sanity checks (Step 4 below) plus the bulk-listing/non-Latin filters — nothing checks that a listing's title actually names the searched product anymore, so unrelated products/brands can appear (verified: "Samsung Galaxy Buds 4 Pro" now also surfaces a "Noise Air Buds Pro 4" listing). The title-matching helpers (`_required_tokens`, `_matches_required_tokens`, `_token_pattern`) and the accessory filter (`_is_accessory`) are still used — unchanged — in the separate route-building stage (`build_routes_for_token`), which was not part of this change.
...
- **Myntra, but only from the deployed backend — new, found 2026-07-22.** Every Myntra link (not just the `/mailers/` ones originally reported) works correctly from a normal machine, but the production server (`dealo-backend.onrender.com`) gets served a "Site Maintenance" placeholder page instead of the real one, every time. Confirmed via `/openapi.json` that the deployed backend is the same current codebase, not stale code — this is Myntra treating Render's server IP differently, the same category of block as Flipkart/Nykaa/AJIO/Croma above, just newly surfaced and specific to where the request comes from rather than blocking everywhere. Mitigated (not fixed) 2026-07-22: `_BAD_TITLE_MARKERS` now rejects "site maintenance" as a title, so the code falls through to reading words straight out of the pasted URL instead of trusting the fake page — this recovers the correct product whenever the link's own slug has real words in it (verified: the reported `/mailers/.../stylecast-x-revolte.../36820535/buy` link now resolves correctly even simulating the blocked response). A link with no descriptive words in its path (a bare ID with nothing else) would still have nothing to fall back to. Closing this fully needs the same paid proxy/IP-rotation fix as the other blocked vendors.

All of the above look like IP- or network-level bot detection (the kind that checks whether a request is coming from a real residential internet connection vs. a data-center server), not something a better user-agent string or extra headers reliably gets around. The only real fix is routing requests through a paid proxy/scraping service — which is exactly the category of tool the old, deleted Zyte integration was. That's a budget decision, not a code one — flagging it here rather than silently deciding either way.
